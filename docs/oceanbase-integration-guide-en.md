...
```sql
CREATE VECTOR INDEX idx_{collection_name}_embedding ON {collection_name}(embedding)
...
- **Construction Parameters**:
  ...
  - `ef_search=64`: Search depth during queries

#### Vector Search Optimization

```go
// Use APPROXIMATE keyword for approximate search
query := fmt.Sprintf(`
    SELECT vector_id, content, metadata, embedding,
           VECTOR_DISTANCE(embedding, ?) as distance,
           1 - VECTOR_DISTANCE(embedding, ?) as similarity_score
    FROM %s
    WHERE VECTOR_DISTANCE(embedding, ?) < ?
    ORDER BY distance ASC
    LIMIT ?
    APPROXIMATE`, collectionName)
```
...
- [Coze Studio Project Repository](https://github.com/coze-dev/coze-studio) d
