...
func (s *oceanbaseSearchStore) Retrieve(ctx context.Context, query string, opts ...retriever.Option) ([]*schema.Document, error) {
	...
	queryContainsName := false
	queryLower := strings.ToLower(query)
	namePatterns := []string{"张三", "李四", "王五", "赵六", "孙七", "周八", "吴九", "郑十"}
	for _, name := range namePatterns {
		if strings.Contains(queryLower, strings.ToLower(name)) {
			queryContainsName = true
			break
		}
	}
	...
	if queryContainsName {
		foundTargetPerson := false
		for _, doc := range documents {
			contentLower := strings.ToLower(doc.Content)
			for _, name := range namePatterns {
				if strings.Contains(contentLower, strings.ToLower(name)) && strings.Contains(queryLower, strings.ToLower(name)) {
					foundTargetPerson = true
					break
				}
			}
			if foundTargetPerson {
				break
			}
		}

		if !foundTargetPerson {
			logs.CtxInfof(ctx, "Query contains specific person name but no matching data found, returning empty results")
